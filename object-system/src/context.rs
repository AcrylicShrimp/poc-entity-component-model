use crate::{
    ...
    ContextActionItem, ContextProxy, ContextResult,
};
...
pub struct Context {
    ...
    controller_storage: RefCell<ControllerStorage>,
}
...
impl Context {
    pub fn new() -> Self {
        Self {
            ...
            controller_storage: RefCell::new(ControllerStorage::new()),
        }
    }
    ...
    fn handle_context_result(&mut self, mut result: ContextResult) {
        ...
        let mut controller_storage = self.controller_storage.borrow_mut();
            ...
            for action in result.action_queue {
                match action {
                    ContextActionItem::RemoveObject { object_id } => {
                        controller_storage.detach_controller(object_id, &mut ctx);
                    ...
                    ContextActionItem::AttachController {
                        ...
                        controller_storage.attach_controller(object_id, controller, &mut ctx);
                    }
                    ContextActionItem::DetachController { object_id } => {
                        controller_storage.detach_controller(object_id, &mut ctx);
                    }
                    ContextActionItem::ListenOnUpdate { object_id } => {
                        controller_storage.listen_on_update(object_id);
                    }
                    ContextActionItem::UnlistenOnUpdate { object_id } => {
                        controller_storage.unlisten_on_update(object_id);
                    }
                    ContextActionItem::ListenOnLateUpdate { object_id } => {
                        controller_storage.listen_on_late_update(object_id);
                    }
                    ContextActionItem::UnlistenOnLateUpdate { object_id } => {
                        controller_storage.unlisten_on_late_update(object_id);
                    }
                }
            }
